#ifndef _HIPPALGORITHM_MATHOPS_LINSPACED_H_
#define _HIPPALGORITHM_MATHOPS_LINSPACED_H_
...
#define _HIPP_TEMPARG LinSpaced<T>::

_HIPP_TEMPHD
_HIPP_TEMPARG LinSpaced(value_t start, diff_t n, value_t stride ) noexcept 
:_start(start), _stride(stride), _n(n)
{}

_HIPP_TEMPHD 
template<typename It>
auto _HIPP_TEMPARG operator>>(It b) const -> const LinSpaced & {
    value_t cur_val = _start;
...
_HIPP_TEMPHD 
template<typename Container>
auto _HIPP_TEMPARG get_to(Container &c) const -> const LinSpaced & {
    c.clear();
    value_t cur_val = _start;
    for(diff_t i=0; i<_n; ++i){
        c.push_back( cur_val );
...
#endif	//_HIPPALGORITHM_MATHOPS_LINSPACED_H_