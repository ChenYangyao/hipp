...
public:
    ...
    index_t brute_force_count_rect(const vector<node_t> &nds, 
        balltree_t::rect_t r) 
    {
        index_t cnt {};
        for(auto &n: nds){
    ...
    index_t brute_force_count_sphere(const vector<node_t> &nds, 
...
for(int i_repeat=0; i_repeat<n_repeat_min; ++i_repeat){
    ...
    for(auto &pt: pts1){
        for(auto &sz: szs){
            auto cnt = tr.count_rect({pt-sz, pt+sz});
...
for(int i_repeat=0; i_repeat<n_repeat_min; ++i_repeat){
    ...
    for(auto &pt: pts){
        ...
        for(auto sz: {0.1f, 0.2f, 0.3f, 0.5f}){
            auto cnt = tr.count_rect({pt+0.05f, pt+sz});
            EXPECT_EQ(cnt, 0);
        }
...
for(int i_repeat=0; i_repeat<n_repeat_min; ++i_repeat){
    ...
    for(auto &n: nds){
        if( n.size() != 1 ) continue;
        for(auto sz: {0.1f, 0.2f, 0.3f, 0.5f}){
            auto cnt = tr.count_rect({n.center()-sz, n.center()+sz});
            EXPECT_GE(cnt, 1);
            EXPECT_LE(cnt, n_pts);
        }
    ...
    for(auto &pt: pts){
        for(auto sz: {0.1f, 0.2f, 0.3f, 0.5f, 1.0f, 2.0f}){
            ...
            auto cnt = tr.count_rect(r),
                cnt_dst = brute_force_count_rect(nds, r);
...
for(int i_repeat=0; i_repeat<n_repeat_min; ++i_repeat){
    ...
    for(auto &pt: pts1){
        for(auto &sz: szs){
            auto cnt = tr.count_sphere({pt, sz});
            EXPECT_EQ(cnt, 0) << "Point " << pt << ", query size " << sz;
        }
    }
}}
...
for(int i_repeat=0; i_repeat<n_repeat_min; ++i_repeat){
    ...
    for(auto &pt: pts){
        ...
        for(auto sz: {0.1f, 0.2f, 0.3f, 0.5f}){
            auto cnt = tr.count_sphere({pt+0.5f, sz});
            EXPECT_EQ(cnt, 0);
        }
...
for(int i_repeat=0; i_repeat<n_repeat_min; ++i_repeat){
    ...
    for(auto &n: nds){
        ...
        for(auto sz: {0.1f, 0.2f, 0.3f, 0.5f}){
            auto cnt = tr.count_sphere({n.center(), sz});
            EXPECT_GE(cnt, 1);
            EXPECT_LE(cnt, n_pts);
        }
    ...
    for(auto &pt: pts){
        for(auto sz: {0.1f, 0.2f, 0.3f, 0.5f, 1.0f, 2.0f}){
            ...
            auto cnt = tr.count_sphere(s),
                cnt_dst = brute_force_count_sphere(nds, s);
